          ...
          <tbody>
            {snapshots.map((s) => (
              <tr key={s.id} className="text-center">
                <td className="px-4 py-2 border">{new Date(s.created_at).toLocaleString()}</td>
                <td className="px-4 py-2 border font-semibold">{s.risk_level.toUpperCase()}</td>
                <td className="px-4 py-2 border">{(s.confidence * 100).toFixed(0)}%</td>
                <td className="px-4 py-2 border">{s.engine_version}</td>
              </tr>
            ))}
          </tbody>
        </table>
