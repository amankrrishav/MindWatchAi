...
export default function ExplanationPanel({ reasons }: { reasons: string[] }) {
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-md w-full max-w-3xl mt-6">
      <button onClick={() => setOpen(!open)} className="text-sm font-medium text-blue-600 hover:underline mb-2">
      ...
      {open && (
        <>
          ...
          <p className="text-xs text-gray-500 mt-3">These factors were detected based on recent responses and changes.</p>
        </>
      )}
    </div>
  );
