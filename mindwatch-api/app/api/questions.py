"""Questions API: next question, submit answer, skip."""
...
def get_next_question(
    ...
    questions = db.query(HumanQuestion).filter_by(active=True).all()
    question_dicts = [
        {"id": q.id, "clinical_key": q.clinical_key, "question_text": q.question_text, "risk_level": q.risk_level}
        for q in questions
    ]
    ...
    q = selected[0]
    if not state:
        state = QuestionGuardrailState(user_id=user_id, questions_today=0, skips_today=0)
        db.add(state)
    state.last_question_at = datetime.utcnow()
    state.questions_today = (state.questions_today or 0) + 1
    state.updated_at = datetime.utcnow()
    db.commit()

    return build_question_card(question_id=q["id"], question_text=q["question_text"])
...
def submit_answer(
    ...
    question = db.query(HumanQuestion).filter_by(id=question_id, active=True).first()
    if not question:
        raise HTTPException(status_code=404, detail="Question not found")

    existing = db.query(HumanAnswer).filter_by(user_id=user_id, question_id=question_id).first()
    if existing:
        raise HTTPException(status_code=409, detail="Question already handled")

    answer = HumanAnswer(user_id=user_id, question_id=question_id, answer_key=payload.answer_key)
    db.add(answer)

    mapping = (
        db.query(AnswerPHQMapping)
        .filter_by(clinical_key=question.clinical_key, answer_key=payload.answer_key)
        .first()
    )
    if mapping:
        db.add(PHQ9Label(user_id=user_id, score=mapping.phq_score))
...
def skip_question(
    ...
    db.add(HumanAnswer(user_id=user_id, question_id=question_id, answer_key="skipped"))
    ...
    state.skips_today = (state.skips_today or 0) + 1
